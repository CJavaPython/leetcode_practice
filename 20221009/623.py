...
Given the integer depth, for each not null tree node cur at the depth depth - 1,
 ...
 depth depth - 1 at all, then create a tree node with value val as the new root 
...
class Solution:
    def addOneRow(self, root: Optional[TreeNode], val: int, depth: int) -> Optional[TreeNode]:
        return 0
s = Solution()
s.addOneRow(root = [4,2,6,3,1,5], val = 1, depth = 2) # output : [4,1,1,2,null,null,6,3,1,5]
s.addOneRow(root = [4,2,None,3,1], val = 1, depth = 3) # output :[4,2,null,1,1,3,null,null,1]